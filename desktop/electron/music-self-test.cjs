...
void (async () => {
  ...
  try {
    ...
    const accountFetcher = async (url, init) => {
      ...
      if (url.includes('/user/playlist')) return { ok: true, json: async () => ({ code: 200, playlist: [{ id: 88, name: '我的收藏', trackCount: 1, userId: 99, creator: { userId: 99, nickname: '自检用户' } }] }) };
      if (url.includes('/playlist/track/all')) return { ok: true, json: async () => ({ code: 200, songs: [{ id: 7, name: '账号歌曲', ar: [{ name: '账号歌手' }], al: { name: '账号专辑' }, dt: 210000 }] }) };
