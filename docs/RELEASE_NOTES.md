Loom 0.1.5 修复 Windows 端音乐服务固定使用 3000 端口的问题。服务现在会使用系统分配的空闲端口，避免端口冲突导致网易云扫码登录无法启动；扫码失败时也会保留实际错误信息。音乐发现页的插画卡片恢复固定比例，最大化窗口后不会随窗口高度被拉长。

## 下载与安装

- Windows x64：下载 `Loom-Setup-0.1.5-x64.exe` 并按安装向导操作。支持应用内检查更新，下载、取消下载与重启安装均由用户确认。
